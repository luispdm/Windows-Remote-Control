...
namespace ServerSide
{
    ...
    public class ClipMngrClient
    {
        ...
        public String Status
        {
            ...
            set
            {
                if (value != _status)
                {
                    ...
                    {
                        window.tblk_clip.Text = _status;
                    }));
                }
            }
        }
        //lato pc attivo se vi è una caduta di connessione si deve ignorare, sarà il pc passivo a richiedere connessione.

        //  GLOBAL VARIABLES FOR STOPPING AND MANAGING THREADS
        // one boolean, two monitors
        private static volatile bool work_;
        private static Object monClip = new Object();
        private static Object monSend = new Object();
        private static Object SendStop = new Object();
        ...
        private ManualResetEvent hasread = new ManualResetEvent(true);
        private ManualResetEvent fileclosed;
        private ManualResetEvent finitocollegamento = new ManualResetEvent(false);
        ...
        private bool icos;  //is closed (by) other side
        private string _status;

        //costruttore per creare da zero un client(instauro nuova connessione)
        public ClipMngrClient(string hname, int pr, MainWindow w)
        {
            try
            {
                icos = false;
                ...
                try
                {
                    ...
                    client.BeginConnect(hname, pr, new AsyncCallback(endconnect), ob);

                }
                catch (Exception e)
                {
                    window.Dispatcher.BeginInvoke((Action)(() => { MessageBox.Show("Errore di connessione"); }));
                ...
                System.IO.Directory.CreateDirectory(pathS);   //per poter creare dei file in C: , bisogna creare prima una sottocartella, xk nn si dispone dei permessi di amministratore
                filen = 0;
        ...
        private void endconnect(IAsyncResult ar)
        {
            ...
            catch (Exception e)
            {
                goodend = false;
                Status = "disconnessa";
                window.Dispatcher.BeginInvoke((Action)(() => { MessageBox.Show("Attenzione, clipboard non connessa!"); }));
            }
            finally
            {

                finitocollegamento.Set();
        ...
        //costruttore a cui passare il client restituito da una precedente accept (lato ClientSide)
        public ClipMngrClient(TcpClient c, MainWindow window)
        {
            this.window = window;
            try
            {
        ...
        public void startReceive()
        {
            ...
            while (work_)
            {
                //creiamo un archivio dove comprimere i file da inviare
                ...
                try
                {
                    ...
                    //ricezione delle dimensioni
                    int i = 0;
                    int letto = nsr.Read(bb, 0, bb.Length);
                    lf = BitConverter.ToUInt64(bb, 0);
                    if (letto == 0)
                    {//se Read torna 0 la socket è stata chiusa (dall'altro endpoint)
                        fS.Close();
                        fS.Dispose();
                        mS.Close();
                        mS.Dispose();
                ...
                catch (Exception excc)
                {
                    ...
                    //if (!work_)
                    //{//se work è false allora il thread di chiusura sta aspettando conferma
                    //    //quindi segnalo che ho chiuso il file
                    //}
                    fileclosed.Set();
                    break;
                }
                ...
                if (ft == FileType.zip && work_)
                {
                    ...
                    {
                        MessageBox.Show("Impossibilie incollare il file - Path troppo lungo");
                        continue;
                    }
        ...
        {
            if (client != null && Status != "disconnessa" && client.Connected)
            {
        ...
        public void SendFiles()
        {
            ...
            if (Monitor.TryEnter(monSend))
            {
                ...
                try
                {
                    ...
                    bool goforw = true;
                    if ((scl[0].Contains(pathS) /*&& isclient*/))
                    {
                    ...
                    else
                    {
                        ...
                        //controlla la scelta utente
                        if (!goforw)
                        {
                            //non fare nulla
                        }
                        else
                        {//zipping e invio dei file in Clipboard
                            ...
                            //se sono ancora connesso, allora mando il file all'utente
                            if (work_)
                            {
            ...
            if (work_) //è ancora attivo
            {
                ...
                if (devochiudere)//se è vero ho deciso di interrompere l'invio
                {
                    try
                    {
                        ...
                        fileclosed.WaitOne();
                        this.Status = "disconnessa";

                        //cancello tutti i files temporanei
        ...
        //adatto la libreria alla compressione ricorsiva di cartelle e sottocartelle
